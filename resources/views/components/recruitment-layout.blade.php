...
@php
    ...
    $siteName = $branding['site_name'] ?? 'درخواست استخدام';
    ...
    $recruitmentTitle = $texts['recruitment_title'] ?? 'درخواست استخدام';
@endphp
...
<head>
    ...
    <title>{{ $title ? $title . ' — ' : '' }}{{ $recruitmentTitle }}</title>
    @if($faviconUrl)
...
<body class="font-sans text-sm md:text-base cyber-bg {{ $scanlines }} text-slate-900 dark:text-slate-100 antialiased min-h-full flex flex-col">
    @if (session()->has(\App\Http\Middleware\EnsureRecruitmentAuth::CONTACT_MOBILE_KEY))
        <header class="cyber-panel border-b border-slate-200 dark:border-cyan-500/20">
            <div class="max-w-4xl mx-auto px-4 h-16 flex items-center justify-between gap-4">
                <a href="{{ route('recruitment.dashboard') }}" class="flex items-center gap-2 font-bold text-slate-800 dark:text-accent">
                    @if($logoUrl)
                ...
                <nav class="flex items-center gap-1 sm:gap-3 text-sm">
                    <a href="{{ route('recruitment.dashboard') }}" class="px-2 py-1 rounded text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-cyan-500/10 {{ request()->routeIs('recruitment.dashboard') ? 'text-cyan-700 dark:text-cyan-400 font-medium' : '' }}">پنل من</a>
                    <button
                    ...
                    <form method="POST" action="{{ route('recruitment.logout') }}">
                        @csrf
