<x-layouts.guest title="تأیید کد">
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-6 md:p-8">
        ...
        <form method="POST" action="{{ route('portal.otp.verify') }}" class="space-y-4">
            @csrf
            ...
            <x-button type="submit" variant="primary" class="w-full">ورود به پورتال</x-button>
        </form>

        <p class="text-center mt-4 text-sm">
            <a href="{{ route('portal.login') }}" class="text-blue-600 hover:underline">تغییر شماره موبایل</a>
        </p>
    </div>
</x-layouts.guest>
