@php
    $importConfig = [
        'uploadUrl' => route('admin.persons.import.store'),
        'processUrlTemplate' => route('admin.persons.import.process', ['importId' => '__IMPORT_ID__']),
    ];
@endphp

<x-modal name="import-persons-csv" title="بارگذاری پرسنل از CSV" maxWidth="2xl">
    <div
    ...
    >
        <div class="rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/40 p-4 text-sm text-slate-600 dark:text-slate-300 leading-6">
            <p>فایل خروجی کاربران وردپرس (یا CSV با ستون‌های مشابه) را بارگذاری کنید.</p>
            <p class="mt-2">فیلدها به‌صورت خودکار به فرم استخدام نگاشت می‌شوند. ستون‌های اضافی برای تکمیل بعدی در پرونده ذخیره می‌شوند.</p>
        </div>
        ...
        <div x-show="completed" class="rounded-lg border border-emerald-200 dark:border-emerald-800/50 bg-emerald-50 dark:bg-emerald-950/20 p-4 text-sm text-emerald-800 dark:text-emerald-200" style="display: none;">
            بارگذاری کامل شد. <span x-text="`${imported} پرسنل جدید و ${updated} به‌روزرسانی شد.`"></span>
        </div>
        ...
        <div class="flex items-center justify-end gap-2 pt-2">
            <button
                type="button"
                @click="completed ? reloadPage() : window.dispatchEvent(new CustomEvent('close-modal', { detail: 'import-persons-csv' }))"
